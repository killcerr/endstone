...
- `Level.recipes`, a snapshot of the crafting recipes the server has loaded, also reachable as `Server.recipes`. Each entry is a `ShapedRecipe`, `ShapelessRecipe`, `StonecuttingRecipe`, `CookingRecipe` (a `FurnaceRecipe`, `BlastingRecipe`, `SmokingRecipe` or `CampfireRecipe`), `BrewingMixRecipe`, `BrewingContainerRecipe`, `SmithingTransformRecipe`, `SmithingTrimRecipe` or `ComplexRecipe`, and reports its `recipe_id`, crafting `tag`, `result` and `ingredients`. Shaped recipes add `width` and `height`; cooking and stonecutting recipes add `input` and `input_choice`; brewing recipes add `input` and `reagent`; smithing recipes add `template`, `base` and `addition`. Vanilla brewing mixes have no JSON identifier, so Endstone synthesises one from the mix. Stonecutter recipes use the `stonecutter` tag; Bedrock stores them as shapeless recipes, and Endstone classifies them by that tag. Shaped, shapeless, stonecutting and cooking recipes also report `priority` (default `0`); when several recipes match, Bedrock prefers the higher one. Brewing and smithing recipes have no priority of their own and report `0`. Shaped, shapeless, stonecutting and cooking recipes also report `unlocking_requirement`: a `RecipeUnlockingRequirement` that is either a `UnlockingContext` (`ALWAYS_UNLOCKED`, `PLAYER_IN_WATER`, `PLAYER_HAS_MANY_ITEMS`) or a list of items to pick up. Plugin recipes default to `ALWAYS_UNLOCKED`. Brewing and smithing recipes are not in the recipe book, so the requirement is unused.
- `Server.register_recipe()` and `Server.unregister_recipe()` for adding and removing crafting recipes at runtime, including brewing mix and container recipes. Call `Server.update_recipes()` after a successful change to resend the crafting data to every connected player, or `Player.update_recipes()` to update one player.
