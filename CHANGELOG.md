...
- Fixed `from endstone import ItemRegistry` (and `EnchantmentRegistry`, `ActorTypeRegistry`) raising `AttributeError`. These names were re-exported but never existed; obtain a registry through `Server.get_registry()`.
- Fixed the server list entry losing its last two fields whenever a plugin listened for `ServerListPingEvent`, or the ping was otherwise answered by Endstone. The reply was rebuilt from a fixed list of fields, dropping whether the world is an editor world and whether it is hardcore; any field Endstone does not itself expose is now passed through untouched.
