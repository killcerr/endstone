...
- Added `EffectType` and `PotionType` to the registry API. Both can now be looked up by name with `EffectType.get("minecraft:speed")` / `PotionType.get("minecraft:potion_type:water")` and enumerated with `server.get_registry(EffectType)` / `server.get_registry(PotionType)`, which report what the server actually supports, and each entry carries a `translation_key`. The existing constants (`EffectType.SPEED`, `PotionType.STRONG_HEALING`, ...) are unchanged.
...
- **BREAKING**: The last shared_ptr-backed types are handed out as handles as well. `Scheduler::runTask()`, `runTaskLater()`, `runTaskTimer()`, `runTaskAsync()`, `runTaskLaterAsync()` and `runTaskTimerAsync()` return `Nullable<Task>`; `CommandMap::registerCommand()` takes `NotNull<Command>` and `CommandMap::getCommand()` returns `Nullable<Command>`; `PluginCommand::setExecutor()` takes `Nullable<CommandExecutor>`, where a null handle still falls back to the owning plugin; and `MapView::getRenderers()`, `addRenderer()` and `removeRenderer()` use `NotNull<MapRenderer>`. C++ plugins pass the `shared_ptr` they already hold; it converts on its own. In Python, `Scheduler.run_task()` is now correctly typed as returning `Task | None`, so a type checker flags the unchecked `None` that was previously hidden.
