...
public:
    explicit EndstoneGameRule(const ::GameRule &game_rule);
    [[nodiscard]] Identifier<GameRule> getId() const override;
    [[nodiscard]] std::string getTranslationKey() const override;
    [[nodiscard]] const ::GameRule &getHandle() const;

private:
    const ::GameRule &game_rule_;
    std::string key_;
};
